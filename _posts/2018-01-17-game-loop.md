...
In the case of MonoGame (Xaml), application initialisation is as follows *(note, many lines removed for sake of brevity)*:
...
`Game1` above is the start of your game code. The MonoGame (Core) variant is similar (though simpler):
...
Whilst all that is a considerable hindrance, it pales in comparison to Unity. I spent a good while trying to decipher the blackbox infrastructure of Unity. I suspect that because it's proprietary and partly because there's a lot of native interop, I didn't really succeed. If you want DI in Unity, you should probably just check out [Zenject](https://github.com/modesttree/Zenject) and be done with it.

We'll consider these points problem number two.
...
We'll call this problem number 4.
...
Given this article has already hit over 2000 words however, write-ups on how I went about solving these problems I'll break out into a mini-series. Subscribe or check back for solutions on each of the problems listed above!